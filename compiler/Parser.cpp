...
struct LocalSyntax : Object {
    LocalSyntax(size_t start, size_t end, String location) : start(start), end(end), location(location) {}
    size_t start;
    size_t end;
    String location;
};
...
struct AsSyntax : Object {
    AsSyntax(size_t start, size_t end, String name, TypeSyntax type) : start(start), end(end), name(name), type(type) {}
    size_t start;
    size_t end;
    String name;
    TypeSyntax type;
...
struct IsSyntax : Object {
    IsSyntax(size_t start, size_t end, String name, TypeSyntax type) : start(start), end(end), name(name), type(type) {}
    size_t start;
    size_t end;
    String name;
    TypeSyntax type;
...
struct DeInitSyntax : Object {
    DeInitSyntax(size_t start, size_t end, RoutineSyntax routine) : start(start), end(end), routine(routine) {}
    size_t start;
    size_t end;
    RoutineSyntax routine;
};
...
struct InitSyntax : Object {
    InitSyntax(size_t start, size_t end, RoutineSyntax routine) : start(start), end(end), routine(routine) {}
    size_t start;
    size_t end;
    RoutineSyntax routine;
};
...
struct ExportSyntax : Object {
    ...
    ExportSyntax(DeInitSyntax _DeInitSyntax) : _tag(DeInit) { _DeInit = _DeInitSyntax; }
...
struct Parser : Object {
    ...
    Vector<String> initialize_keywords_index(Page* _rp) {
        ...
        keywords_builder.add(String(Page::get(this), "as"));
    ...
    Result<ExportSyntax, ParserError> parse_export(Page* _rp, Page* _ep) {
        ...
        }
        {
            ...
            else
            {
                auto node = node_result._Ok;
                return Result<ExportSyntax, ParserError> { ._tag = Result<ExportSyntax, ParserError>::Ok, ._Ok = 
                    ExportSyntax(DeInitSyntax(node))
                };
            }
        }
        {
    ...
    Result<ReturnsSyntax, ParserError> parse_returns(Page* _rp, Page* _ep) {
        ...
        if (parameters_result._tag == Result<ParameterSetSyntax, ParserError>::Error)
        {
            switch (parameters_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<ReturnsSyntax, ParserError> { ._tag = Result<ReturnsSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(parameters_start, lexer.position, String(_ep, "a valid ParameterSet syntax"))) };
                case ParserError::InvalidSyntax:
        ...
        if (properties_result._tag == Result<Vector<PropertySyntax>, ParserError>::Error)
        {
            switch (properties_result._Error._tag) {
                case ParserError::OtherSyntax:
                    break;
                case ParserError::InvalidSyntax:
                    return Result<ParametersSyntax, ParserError> { ._tag = Result<ParametersSyntax, ParserError>::Error, ._Error = properties_result._Error };
            }
        }
    ...
    Result<ImplementSyntax, ParserError> parse_implement(Page* _rp, Page* _ep) {
        ...
        auto success_implement_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[18]);
        ...
        if (generics_result._tag == Result<GenericArgumentsSyntax, ParserError>::Error)
        {
            switch (generics_result._Error._tag) {
                case ParserError::OtherSyntax:
                    break;
                case ParserError::InvalidSyntax:
                    return Result<FunctionSyntax, ParserError> { ._tag = Result<FunctionSyntax, ParserError>::Error, ._Error = generics_result._Error };
            }
        }
    ...
    Result<InitSyntax, ParserError> parse_init(Page* _rp, Page* _ep) {
        ...
        if (routine_result._tag == Result<RoutineSyntax, ParserError>::Error)
        {
            switch (routine_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<InitSyntax, ParserError> { ._tag = Result<InitSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(routine_start, lexer.position, String(_ep, "a valid Routine syntax"))) };
                case ParserError::InvalidSyntax:
                    return Result<InitSyntax, ParserError> { ._tag = Result<InitSyntax, ParserError>::Error, ._Error = routine_result._Error };
            }
        }
        ...
        auto ret = InitSyntax(start, end, routine);
    ...
    Result<DeInitSyntax, ParserError> parse_deinit(Page* _rp, Page* _ep) {
        ...
        if (routine_result._tag == Result<RoutineSyntax, ParserError>::Error)
        {
            switch (routine_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<DeInitSyntax, ParserError> { ._tag = Result<DeInitSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(routine_start, lexer.position, String(_ep, "a valid Routine syntax"))) };
                case ParserError::InvalidSyntax:
                    return Result<DeInitSyntax, ParserError> { ._tag = Result<DeInitSyntax, ParserError>::Error, ._Error = routine_result._Error };
            }
        }
        ...
        auto ret = DeInitSyntax(start, end, routine);
        ...
        if (generics_result._tag == Result<GenericArgumentsSyntax, ParserError>::Error)
        {
            switch (generics_result._Error._tag) {
                case ParserError::OtherSyntax:
                    break;
                case ParserError::InvalidSyntax:
                    return Result<ProcedureSyntax, ParserError> { ._tag = Result<ProcedureSyntax, ParserError>::Error, ._Error = generics_result._Error };
            }
        }
    ...
    Result<OperatorSyntax, ParserError> parse_operator(Page* _rp, Page* _ep) {
        ...
        auto success_operator_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[30]);
        ...
        if (returns_result._tag == Result<ReturnsSyntax, ParserError>::Error)
        {
            switch (returns_result._Error._tag) {
                case ParserError::OtherSyntax:
                    break;
                case ParserError::InvalidSyntax:
                    return Result<RoutineSyntax, ParserError> { ._tag = Result<RoutineSyntax, ParserError>::Error, ._Error = returns_result._Error };
            }
        }
        ...
        if (throws_result._tag == Result<ThrowsSyntax, ParserError>::Error)
        {
            switch (throws_result._Error._tag) {
                case ParserError::OtherSyntax:
                    break;
                case ParserError::InvalidSyntax:
                    return Result<RoutineSyntax, ParserError> { ._tag = Result<RoutineSyntax, ParserError>::Error, ._Error = throws_result._Error };
            }
        }
    ...
    Result<InstructionSyntax, ParserError> parse_instruction(Page* _rp, Page* _ep) {
        ...
        auto success_instruction_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[20]);
    ...
    Result<IntrinsicSyntax, ParserError> parse_intrinsic(Page* _rp, Page* _ep) {
        ...
        auto success_intrinsic_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[21]);
    ...
    Result<MacroSyntax, ParserError> parse_macro(Page* _rp, Page* _ep) {
        ...
        auto success_macro_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[26]);
        if (!success_macro_1) {
        ...
        if (rule_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (rule_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<MacroSyntax, ParserError> { ._tag = Result<MacroSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(rule_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<ModuleSyntax, ParserError> parse_module(Page* _rp, Page* _ep) {
        ...
        auto success_module_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[28]);
        if (!success_module_1) {
    ...
    Result<LetSyntax, ParserError> parse_let(Page* _rp, Page* _ep) {
        ...
        auto success_let_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[24]);
        if (!success_let_1) {
    ...
    Result<MutableSyntax, ParserError> parse_mutable(Page* _rp, Page* _ep) {
        ...
        auto success_mutable_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[29]);
    ...
    Result<BindingSyntax, ParserError> parse_binding(Page* _rp, Page* _ep) {
        ...
        if (operation_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (operation_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<BindingSyntax, ParserError> { ._tag = Result<BindingSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(operation_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<CatchSyntax, ParserError> parse_catch(Page* _rp, Page* _ep) {
        ...
        auto success_catch_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[2]);
        if (!success_catch_1) {
        ...
        if (condition_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (condition_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<CatchSyntax, ParserError> { ._tag = Result<CatchSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(condition_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
        ...
        if (handler_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (handler_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<CatchSyntax, ParserError> { ._tag = Result<CatchSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(handler_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<DropSyntax, ParserError> parse_drop(Page* _rp, Page* _ep) {
        ...
        if (handler_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (handler_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<DropSyntax, ParserError> { ._tag = Result<DropSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(handler_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<BreakSyntax, ParserError> parse_break(Page* _rp, Page* _ep) {
        ...
        auto success_break_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[1]);
        if (!success_break_1) {
    ...
    Result<LoopSyntax, ParserError> parse_loop(Page* _rp, Page* _ep) {
        ...
        auto success_loop_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[25]);
        if (!success_loop_1) {
    ...
    Result<ValueSyntax, ParserError> parse_value(Page* _rp, Page* _ep) {
        ...
        if (value_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (value_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<ValueSyntax, ParserError> { ._tag = Result<ValueSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(value_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<IfSyntax, ParserError> parse_if(Page* _rp, Page* _ep) {
        ...
        if (condition_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (condition_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<IfSyntax, ParserError> { ._tag = Result<IfSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(condition_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
        ...
        if (match_result._tag == Result<IsSyntax, ParserError>::Error)
        {
            switch (match_result._Error._tag) {
                case ParserError::OtherSyntax:
                    break;
                case ParserError::InvalidSyntax:
                    return Result<IfSyntax, ParserError> { ._tag = Result<IfSyntax, ParserError>::Error, ._Error = match_result._Error };
            }
        }
        ...
        if (alias_result._tag == Result<AsSyntax, ParserError>::Error)
        {
            switch (alias_result._Error._tag) {
                case ParserError::OtherSyntax:
                    break;
                case ParserError::InvalidSyntax:
                    return Result<IfSyntax, ParserError> { ._tag = Result<IfSyntax, ParserError>::Error, ._Error = alias_result._Error };
            }
        }
        ...
        if (consequent_result._tag == Result<ActionSyntax, ParserError>::Error)
        {
            switch (consequent_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<IfSyntax, ParserError> { ._tag = Result<IfSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(consequent_start, lexer.position, String(_ep, "a valid Action syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<IsSyntax, ParserError> parse_is(Page* _rp, Page* _ep) {
        ...
        auto success_is_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[17]);
        if (!success_is_1) {
        ...
        if (name != nullptr) {
            if (!this->is_identifier(*name)) {
            return Result<IsSyntax, ParserError> { ._tag = Result<IsSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(start_name, lexer.position, String(_ep, "an identifier"))) };
            }
        }
        else {
            return Result<IsSyntax, ParserError> { ._tag = Result<IsSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(start_name, lexer.position, String(_ep, "an identifier"))) };
        ...
        auto success_colon_3 = this->lexer.parse_colon(_rp);
        if (!success_colon_3) {
            return Result<IsSyntax, ParserError> { ._tag = Result<IsSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(start_colon_3, lexer.position, String(_ep, "a colon or a line feed"))) };        }
        ...
        if (type_result._tag == Result<TypeSyntax, ParserError>::Error)
        {
            switch (type_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<IsSyntax, ParserError> { ._tag = Result<IsSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(type_start, lexer.position, String(_ep, "a valid Type syntax"))) };
                case ParserError::InvalidSyntax:
        ...
        auto ret = IsSyntax(start, end, *name, type);
    ...
    Result<AsSyntax, ParserError> parse_as(Page* _rp, Page* _ep) {
        ...
        auto success_as_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[0]);
        if (!success_as_1) {
        ...
        auto success_colon_3 = this->lexer.parse_colon(_rp);
        if (!success_colon_3) {
            return Result<AsSyntax, ParserError> { ._tag = Result<AsSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(start_colon_3, lexer.position, String(_ep, "a colon or a line feed"))) };        }
        ...
        if (type_result._tag == Result<TypeSyntax, ParserError>::Error)
        {
            switch (type_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<AsSyntax, ParserError> { ._tag = Result<AsSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(type_start, lexer.position, String(_ep, "a valid Type syntax"))) };
                case ParserError::InvalidSyntax:
                    return Result<AsSyntax, ParserError> { ._tag = Result<AsSyntax, ParserError>::Error, ._Error = type_result._Error };
            }
        }
        ...
        auto ret = AsSyntax(start, end, *name, type);
    ...
    Result<ElseSyntax, ParserError> parse_else(Page* _rp, Page* _ep) {
        ...
        if (alternative_result._tag == Result<ActionSyntax, ParserError>::Error)
        {
            switch (alternative_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<ElseSyntax, ParserError> { ._tag = Result<ElseSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(alternative_start, lexer.position, String(_ep, "a valid Action syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<MatchSyntax, ParserError> parse_match(Page* _rp, Page* _ep) {
        ...
        auto success_match_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[27]);
        if (!success_match_1) {
        ...
        if (scrutinee_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (scrutinee_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<MatchSyntax, ParserError> { ._tag = Result<MatchSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(scrutinee_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<CaseSyntax, ParserError> parse_case(Page* _rp, Page* _ep) {
        ...
        auto success_case_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[3]);
        if (!success_case_1) {
        ...
        if (condition_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (condition_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<CaseSyntax, ParserError> { ._tag = Result<CaseSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(condition_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<LambdaSyntax, ParserError> parse_lambda(Page* _rp, Page* _ep) {
        ...
        auto success_lambda_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[23]);
        if (!success_lambda_1) {
        ...
        if (input_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (input_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<LambdaSyntax, ParserError> { ._tag = Result<LambdaSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(input_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<ForSyntax, ParserError> parse_for(Page* _rp, Page* _ep) {
        ...
        auto success_in_3 = this->lexer.parse_keyword(_rp, *this->keywords_index[19]);
        if (!success_in_3) {
    ...
    Result<LabelSyntax, ParserError> parse_label(Page* _rp, Page* _ep) {
        ...
        auto success_label_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[22]);
        if (!success_label_1) {
    ...
    Result<WhileSyntax, ParserError> parse_while(Page* _rp, Page* _ep) {
        ...
        if (condition_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (condition_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<WhileSyntax, ParserError> { ._tag = Result<WhileSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(condition_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<SetSyntax, ParserError> parse_set(Page* _rp, Page* _ep) {
        ...
        if (target_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (target_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<SetSyntax, ParserError> { ._tag = Result<SetSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(target_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
        ...
        if (source_result._tag == Result<OperationSyntax, ParserError>::Error)
        {
            switch (source_result._Error._tag) {
                case ParserError::OtherSyntax:
                    return Result<SetSyntax, ParserError> { ._tag = Result<SetSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(source_start, lexer.position, String(_ep, "a valid Operation syntax"))) };
                case ParserError::InvalidSyntax:
    ...
    Result<LocalSyntax, ParserError> parse_local(Page* _rp, Page* _ep) {
        ...
        if (location != nullptr) {
            if (!this->is_identifier(*location)) {
            return Result<LocalSyntax, ParserError> { ._tag = Result<LocalSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(start_location, lexer.position, String(_ep, "an identifier"))) };
            }
        }
        else {
            return Result<LocalSyntax, ParserError> { ._tag = Result<LocalSyntax, ParserError>::Error, ._Error = ParserError(InvalidSyntax(start_location, lexer.position, String(_ep, "an identifier"))) };
        ...
        auto ret = LocalSyntax(start, end, *location);
    ...
    Result<ReferenceSyntax, ParserError> parse_reference(Page* _rp, Page* _ep) {
        ...
        auto start_age = this->lexer.previous_position;
