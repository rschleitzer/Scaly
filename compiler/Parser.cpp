...
struct IfSyntax : Object {
    IfSyntax(size_t start, size_t end, OperationSyntax condition, IsSyntax* match, ActionSyntax* consequent, ElseSyntax* alternative) : start(start), end(end), condition(condition), match(match), consequent(consequent), alternative(alternative) {}
    size_t start;
    size_t end;
    OperationSyntax condition;
    IsSyntax* match;
...
struct RoutineSyntax : Object {
    RoutineSyntax(size_t start, size_t end, GenericArgumentsSyntax* generics, LifetimeSyntax* lifetime, ParameterSetSyntax* parameters, Vector<AttributeSyntax>* attributes, ReturnsSyntax* returns, ThrowsSyntax* throws, ImplementationSyntax implementation) : start(start), end(end), generics(generics), lifetime(lifetime), parameters(parameters), attributes(attributes), returns(returns), throws(throws), implementation(implementation) {}
    size_t start;
    size_t end;
    GenericArgumentsSyntax* generics;
    LifetimeSyntax* lifetime;
...
struct Parser : Object {
    ...
    Result<RoutineSyntax, ParserError> parse_routine(Page* _rp, Page* _ep) {
        ...
        auto ret = RoutineSyntax(start, end, generics, lifetime, parameters, attributes, returns, throws, implementation);
    ...
    Result<IfSyntax, ParserError> parse_if(Page* _rp, Page* _ep) {
        ...
        ActionSyntax* consequent = consequent_result._tag == Result<ActionSyntax, ParserError>::Error ? nullptr : new(alignof(ActionSyntax), _rp) ActionSyntax(consequent_result._Ok);
        ...
        auto ret = IfSyntax(start, end, condition, match, consequent, alternative);
    ...
    Result<IsSyntax, ParserError> parse_is(Page* _rp, Page* _ep) {
        ...
        auto success_is_1 = this->lexer.parse_keyword(_rp, *this->keywords_index[16]);
        if (!success_is_1) {
