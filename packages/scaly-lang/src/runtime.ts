// Scaly Runtime - executes operations using stdlib definitions
...
export class Runtime {
  ...
  // Load stdlib and build registries
  loadStdlib(stdlibPath: string): { ok: true } | { ok: false; error: string } {
    let content: string
    try {
      content = fs.readFileSync(stdlibPath, 'utf-8')
    } catch (e) {
      return { ok: false, error: `Failed to read stdlib: ${e}` }
    }

    const result = parseAndModelFile(content, stdlibPath)
    if (!result.ok) {
      return { ok: false, error: `Failed to parse stdlib: ${formatModelError(result.error, content)}` }
    }
  ...
  private constructWrapper(typeName: string, tuple: Model.Tuple, scope: Scope): { ok: true; value: Value } | { ok: false; error: string } {
    ...
    // TODO: Get actual field names from type definition
...
export function getRuntime(stdlibPath?: string): Runtime {
  if (!globalRuntime) {
    globalRuntime = new Runtime()
    const defaultPath = stdlibPath || path.join(process.cwd(), 'stdlib.scaly')
    const result = globalRuntime.loadStdlib(defaultPath)
    if (!result.ok) {
