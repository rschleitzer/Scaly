...
(element (itemizedlist listitem)
  (let ((ilevel (length (hierarchical-number-recursive (normalize "itemizedlist"))))
	...
	;; Use attribute-string (not inherited) so nested lists can have their own bullet style
	(mark (attribute-string (normalize "mark") (parent (current-node)))))
    (generic-list-item
     (ILSTEP)
...
(element (orderedlist listitem)
  (let* ((listitems (select-elements (children (parent (current-node)))
	 ...
	 (separator (if (equal? numeration (normalize "loweralpha"))
			" "
			(string-append (gentext-label-title-sep (normalize "orderedlist")) " "))))
    (generic-list-item
     (* (OLSTEP) factor)
     ;; Use simple literal instead of line-field to avoid RTF tab character
...
;; Standard para spacing between simplelist items
	    cell-before-row-margin: (/ %para-sep% 2)
	    cell-after-row-margin: (/ %para-sep% 2)
