...
(define %mono-font-family%
  ...
  ;; DESC
  ;; The name of the font family used in verbatim environments (Courier
  ;; to match PDF).
  ;;
  ;; The values used here are system dependent (you have
  ;; to have the fonts you select) and backend dependent (the backend has
...
(define %hsize-bump-factor%
  ...
  ;;
  ;; Reduced to 1.13 to match ISO PDF heading sizes more closely.
  ;; /DESC
  ;; AUTHOR N/A
  ;; /REFENTRY
  1.13)
...
(define %verbatim-size-factor%
  ...
  ;; PURP Verbatim font scaling factor for inline code
  ;; DESC
  ;; When a monospace font is selected in inline contexts (code, literal),
  ;; the current font size is multiplied by the '%verbatim-size-factor%'.
  ;; Set to 1.0 to disable downscaling - inline monospace should be same size as body text
  ;; to match ISO PDF style.
  ;; /DESC
  ;; AUTHOR N/A
  ;; /REFENTRY
  1.0)

(define %programlisting-size-factor%
  ;; REFENTRY programlisting-size-factor
  ;; PURP Font scaling factor for programlisting blocks
  ;; DESC
  ;; When rendering programlisting blocks, the font size is multiplied
  ;; by the '%programlisting-size-factor%'.
  ;; /DESC
  ;; AUTHOR N/A
  ;; /REFENTRY
  0.9)
...
(define %bf-size%
  ...
  ;; Customized to 11pt to match PDF (PDF uses 10.8pt).
  ;; /DESC
  ;; /REFENTRY
  (case %visual-acuity%
    (("tiny") 8pt)
...
(define %left-margin%
  ...
  ;; Set to 9pi to match ISO PDF text flow width.
  ;; /DESC
  ;; AUTHOR N/A
  ;; /REFENTRY
  9pi)
...
(define %body-start-indent%
  ...
  ;; indentation.
  ;; Changed to 0pt to match ISO PDF style (flush left).
  ;; /DESC
  ;; AUTHOR N/A
  ;; /REFENTRY
  0pt)
