...
(element book 
  (let* ((bookinfo  (select-elements (children (current-node)) 
    ...
    (make sequence
      ...
      (if (not (generate-toc-in-front))
	  ;; Wrap main content in simple-page-sequence for headers/footers
	  ;; but let chapters flow continuously without page breaks
	  (make simple-page-sequence
	    page-n-columns: %page-n-columns%
	    page-number-restart?: %page-number-restart%
