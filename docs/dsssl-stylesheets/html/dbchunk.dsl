...
(define (chunk-element-list)
  (list (normalize "preface")
	...
	(normalize "refentry")
	(normalize "part")
	;; sect1 and section removed to keep chapters on single pages
...
(define (id-based-filename nd)
  (if (and %use-id-as-filename%
	   (or (attribute-string (normalize "id") nd)
	       (attribute-string (normalize "xml:id") nd)))
      (case-fold-down (or (attribute-string (normalize "id") nd)
			  (attribute-string (normalize "xml:id") nd)))
      #f))
